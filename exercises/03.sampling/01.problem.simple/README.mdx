...
- Implement a function that sends a sampling request to the client using
  `agent.mcp.server.createMessage` (the `mcp.server` thing is funny, but
...
```ts
// the mcp.server is how the MCP SDK exposes the underlying server
...
```ts
// this.mcp.server to access the underlying server instance
